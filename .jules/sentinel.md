...
## 2026-04-19 - Fix Arbitrary Field Deletion in getOauthUnlink
**Vulnerability:** The `getOauthUnlink` route used user-supplied input (`provider`) from `req.params` to directly unset properties on the database user object (`user[provider.toLowerCase()] = undefined;`). An attacker could supply arbitrary keys like `password`, `email`, or `tokens`, which would unset those fields and lead to potential account takeover or denial of service.
**Learning:** Taking user-controlled strings and applying them dynamically as property keys on a Mongoose model is highly dangerous and allows attackers to bypass intended application logic via Insecure Direct Object Reference (IDOR) / Arbitrary Field Deletion vulnerabilities.
**Prevention:** Always validate user-supplied keys against an explicit allowlist of known safe fields before dynamically accessing or modifying model properties.
