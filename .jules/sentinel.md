...
## 2026-04-08 - [HIGH] Arbitrary Field Deletion via OAuth Unlink
**Vulnerability:** The `/account/unlink/:provider` route allowed unlinking arbitrary OAuth providers by directly using the user-provided `provider` param to delete properties on the `User` model (`user[provider.toLowerCase()] = undefined`). This allowed attackers to perform Arbitrary Field Deletion / Insecure Direct Object Reference (IDOR) by supplying arbitrary field names like `password`, `email`, or `emailVerified`, effectively destroying their account integrity or causing authentication bypasses.
**Learning:** Using untrusted user input as an object property key to modify or delete properties on sensitive data structures like Mongoose documents is extremely dangerous.
**Prevention:** Always validate user-provided property keys against an explicit allowed list of fields before using them to access or modify object properties.
