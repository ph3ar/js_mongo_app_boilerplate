...
## 2024-05-24 - [HIGH] Arbitrary Field Deletion in OAuth Unlink
**Vulnerability:** The `getOauthUnlink` route previously allowed users to specify the `provider` via a URL parameter (`req.params.provider`). This parameter was used as an arbitrary object key to set properties on the Mongoose user model to undefined: `user[provider.toLowerCase()] = undefined`. An attacker could exploit this Mass Assignment / IDOR vulnerability to nullify crucial properties such as `password` or `email` simply by hitting `/account/unlink/password`.
**Learning:** Naively using `req.params` or `req.body` directly as keys for assigning values to internal state objects exposes systems to arbitrary field manipulation. Without an allowlist, internal objects are vulnerable.
**Prevention:** Always ensure any dynamic assignment to object properties validates keys against a strict allowlist.
